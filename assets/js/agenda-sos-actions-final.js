/* BeautyMove — controlador S.O.S. sem loop de DOM */
(function(){
  'use strict';

  const STATE_KEY='beautymove.mvp.state';
  const read=()=>{
    try{return JSON.parse(localStorage.getItem(STATE_KEY)||'null')||{appointments:[],opportunities:[],transactions:[]};}
  ...
  const notice=msg=>{
    ...
    n.textContent=msg;
    n.hidden=false;
    clearTimeout(window.__bmSosControllerNotice);
    window.__bmSosControllerNotice=setTimeout(()=>n.hidden=true,3500);
  };
  ...
  const servicesOf=o=>{
    ...
    return list.map(s=>({id:s.id,name:s.name,duration:Number(s.duration||s.durationMinutes||0)||30,value:Number(s.value||s.clientPrice||0)}));
  };
  ...
  function ensureAppointment(op,state){
    ...
    const professional=String(op.acceptedBy||op.professional||'').trim();
    if(!professional)return null;

    const linked=state.appointments.filter(a=>a&&a.status!=='cancelado'&&(a.id===op.appointmentId||a.sosOpportunityId===op.id));
    ...
    if(appointment){
      linked.filter(a=>a.id!==appointment.id).forEach(a=>{
        a.status='cancelado';
        a.cancelledAt=new Date().toISOString();
        a.cancelledReason='Duplicidade corrigida automaticamente pela reserva S.O.S.';
      });
      let changed=linked.length>1;
      if(appointment.professional!==professional){appointment.professional=professional;appointment.sosAcceptedBy=professional;changed=true;}
      ...
      const end=time(mins(start)+duration);
      if(appointment.endTime!==end){appointment.endTime=end;changed=true;}
      if(appointment.source!=='sos'){appointment.source='sos';changed=true;}
    ...
    const conflict=state.appointments.some(a=>a&&a.status!=='cancelado'&&a.date===date&&a.professional===professional&&overlaps(mins(start),mins(start)+duration,mins(a.time),mins(a.time)+durationOf(a)));
    if(conflict)return null;

    const id=`apt-sos-${Date.now()}-${Math.random().toString(36).slice(2,7)}`;
    const services=servicesOf(op);
    appointment={
      id,date,time:start,endTime:time(mins(start)+duration),professional,
      client:op.client||'Cliente',services,
      service:op.service||services.map(s=>s.name).join(' + '),
      duration,durationMinutes:duration,value:valueOf(op),status:'agendado',source:'sos',
      sosAcceptedBy:professional,sosAcceptedAt:op.acceptedAt||new Date().toISOString(),sosOpportunityId:op.id
    };
  ...
  function resolveContext(){
    ...
    if(requestedId){
      ...
      if(op){
        ...
        if(appointment||op.acceptedBy||op.appointmentId){
          return{state,opportunity:op,appointment:appointment||apps.find(a=>a&&a.id===op.appointmentId)||null};
        }
      }
    }
    ...
    for(const op of ops){
      ...
      if(client&&timeValue&&text.includes(client)&&text.includes(timeValue)){
        return{state,opportunity:op,appointment:a||ensureAppointment(op,state)};
      }
    }
    return null;
  }
  ...
  function normalizeButtons(){
    ...
    if(!ctx||!ctx.opportunity)return false;

    b.dataset.sosId=ctx.opportunity.id;
    if(ctx.appointment)b.dataset.appointmentId=ctx.appointment.id;

    /* Importante: esta função é chamada apenas quando necessário.
       Não há MutationObserver nem setInterval, evitando ciclo infinito de DOM. */
    b.querySelectorAll('button').forEach(btn=>btn.remove());

    const schedule=document.createElement('button');
    schedule.type='button';
    schedule.className='action-button';
    schedule.textContent='Alterar horário';

    const cancel=document.createElement('button');
    cancel.type='button';
    cancel.className='action-button action-cancel';
    cancel.textContent='Cancelar atendimento';
  ...
  function buildTimeOptions(selected){
    const out=[];
    for(let m=0;m<=1439;m+=30){
      const t=time(m);
      out.push(`<option value="${t}"${t===selected?' selected':''}>${t}</option>`);
    }
    return out.join('');
  }
  ...
  function openReschedule(){
    const ctx=resolveContext();
    if(!ctx||!ctx.appointment){notice('Não foi possível localizar a reserva S.O.S.');return;}

    let modal=document.getElementById('sosRescheduleModal');
    if(!modal){
      modal=document.createElement('div');
      modal.id='sosRescheduleModal';
    ...
    modal.dataset.sosId=ctx.opportunity.id;
    modal.dataset.appointmentId=ctx.appointment.id;
    open('sosRescheduleModal');
  }

  function saveReschedule(){
    const modal=document.getElementById('sosRescheduleModal');
    const id=modal?.dataset?.sosId;
    const timeValue=document.getElementById('sosRescheduleTime')?.value;
    if(!id||!timeValue){notice('Não foi possível identificar o horário.');return;}

    const state=read();
    const op=(state.opportunities||[]).find(o=>o&&o.id===id);
    const appointmentId=modal.dataset.appointmentId;
    const appointment=appointmentId?(state.appointments||[]).find(a=>a&&a.id===appointmentId):null;
    if(!op||!appointment){notice('Atendimento S.O.S. não encontrado.');return;}

    const duration=durationOf(appointment);
    const startMin=mins(timeValue);
    const conflict=(state.appointments||[]).some(a=>a&&a.id!==appointment.id&&a.status!=='cancelado'&&a.date===appointment.date&&a.professional===appointment.professional&&overlaps(startMin,startMin+duration,mins(a.time),mins(a.time)+durationOf(a)));
    ...
    appointment.endTime=time(startMin+duration);
    op.time=timeValue;
    op.endTime=appointment.endTime;
    write(state);
    close('sosRescheduleModal');
    notice('Horário alterado com sucesso.');
    window.setTimeout(()=>location.reload(),150);
  }

  function cancelAppointment(){
    const ctx=resolveContext();
    if(!ctx){notice('Não foi possível localizar o atendimento S.O.S.');return;}
    ...
    opportunity.cancelledReason='Atendimento S.O.S. cancelado pelo salão';
    write(state);
    close('detailsModal');
    location.reload();
  }
  ...
  function bind(){
    document.addEventListener('click',e=>{
      ...
      if(cell){
        window.__bmCurrentSosId=cell.dataset.sosId||null;
        window.__bmCurrentAppointmentId=cell.dataset.appointmentId||null;
        /* A agenda pode abrir a janela de detalhes no mesmo clique.
           Fazemos uma única normalização após a renderização, sem loop. */
        window.setTimeout(normalizeButtons,50);
        return;
      }
      ...
      if(text!=='alterar horário'&&text!=='cancelar atendimento')return;
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      if(text==='alterar horário')openReschedule();
      else cancelAppointment();
    },true);

    /* Não executamos normalizeButtons continuamente. */
