...
(function(){
  ...
  function ensureAppointment(op,state){
    ...
    if(appointment){
      linked.filter(a=>a.id!==appointment.id).forEach(a=>{a.status='cancelado';a.cancelledAt=new Date().toISOString();a.cancelledReason='Duplicidade corrigida automaticamente pela reserva S.O.S.';});
      ...
      op.appointmentId=appointment.id;op.acceptedBy=professional;op.status='resolved';op.endTime=end;
    ...
    appointment={id,date,time:start,endTime:time(mins(start)+duration),professional,client:op.client||'Cliente',services,service:op.service||services.map(s=>s.name).join(' + '),duration,durationMinutes:duration,value:valueOf(op),status:'agendado',source:'sos',sosAcceptedBy:professional,sosAcceptedAt:op.acceptedAt||new Date().toISOString(),sosOpportunityId:op.id};
    state.appointments.push(appointment);
    op.appointmentId=id;op.acceptedBy=professional;op.status='resolved';op.endTime=appointment.endTime;
    write(state);
    return appointment;
  }
  ...
  function normalizeButtons(){
    ...
    const ctx=resolveContext();if(!ctx||!ctx.opportunity)return false;
    ...
    b.append(schedule,cancel);return true;
  }
  ...
  function saveReschedule(){
    ...
    const state=read(),op=(state.opportunities||[]).find(o=>o&&o.id===id),appointmentId=modal.dataset.appointmentId,appointment=appointmentId?(state.appointments||[]).find(a=>a&&a.id===appointmentId):null;if(!op||!appointment){notice('Atendimento S.O.S. não encontrado.');return;}
    const duration=durationOf(appointment),startMin=mins(timeValue),conflict=(state.appointments||[]).some(a=>a&&a.id!==appointment.id&&a.status!=='cancelado'&&a.date===appointment.date&&a.professional===appointment.professional&&overlaps(startMin,startMin+duration,mins(a.time),mins(a.time)+durationOf(a)));if(conflict){notice(`${appointment.professional||'Profissional'} já possui atendimento nesse horário.`);return;}
    appointment.time=timeValue;appointment.endTime=time(startMin+duration);op.time=timeValue;op.endTime=appointment.endTime;write(state);close('sosRescheduleModal');notice('Horário alterado com sucesso.');window.setTimeout(()=>location.reload(),150);
  }
  ...
  function bind(){
    document.addEventListener('click',e=>{
      ...
      if(cell){window.__bmCurrentSosId=cell.dataset.sosId||null;window.__bmCurrentAppointmentId=cell.dataset.appointmentId||null;window.setTimeout(normalizeButtons,50);return;}
    ...
    if(!document.getElementById('bmAgendaSosRenderFinal')){
      const renderScript=document.createElement('script');renderScript.id='bmAgendaSosRenderFinal';renderScript.src='assets/js/agenda-sos-render-final.js?v=20260819-1';document.head.appendChild(renderScript);
    }
  }
