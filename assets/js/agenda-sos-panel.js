...
(function(){
  ...
  function ensureAssets(){
    if(!document.querySelector('link[data-sos-workspace-css]')){const l=document.createElement('link');l.rel='stylesheet';l.href=`${base}css/agenda-sos-workspace.css?v=20260824-5`;l.dataset.sosWorkspaceCss='1';document.head.appendChild(l)}
    if(document.querySelector('script[data-sos-workspace-js]'))return;
    const a=document.createElement('script');a.src=`${base}js/agenda-sos-workspace.js?v=20260824-5`;a.dataset.sosWorkspaceJs='1';a.onload=()=>{if(document.querySelector('script[data-sos-agenda-js]'))return;const b=document.createElement('script');b.src=`${base}js/agenda-sos.js?v=20260824-5`;b.dataset.sosAgendaJs='1';document.body.appendChild(b)};document.body.appendChild(a);
  }
  ensureAssets();
  ...
  function setMainDate(date){const picker=document.getElementById('agendaDatePicker');if(!picker||!date)return;if(picker.value!==date){picker.value=date;picker.dispatchEvent(new Event('change',{bubbles:true}));window.dispatchEvent(new CustomEvent('beautymove:agenda-date-changed',{detail:{date}}))}}
  ...
  function render(){
    ...
    body.innerHTML=ops.map(o=>`<article class="agenda-sos-opportunity" data-sos-id="${esc(o.id)}" tabindex="0">
      <h3>${esc(o.service||'Serviço S.O.S.')}</h3>
      <p><strong>Cliente:</strong> ${esc(o.client||'Cliente')}</p>
      <p><strong>Data:</strong> ${esc(o.date||'')} · <strong>Horário:</strong> ${esc(o.time||'')}</p>
      <p><strong>Especialidade:</strong> ${esc(o.specialty||'')} · <strong>Duração:</strong> ${duration(o.durationMinutes)}</p>
      ${o.value!=null?`<p><strong>Valor:</strong> R$ ${Number(o.value||0).toLocaleString('pt-BR',{minimumFractionDigits:2})}</p>`:''}
      ${o.radius?`<p><strong>Raio:</strong> ${esc(o.radius)}</p>`:''}
      ${o.materials?`<p><strong>Material:</strong> ${esc(o.materials)}</p>`:''}
      ${o.sentTo?`<p><strong>Profissional:</strong> ${esc(o.sentTo)}</p>`:''}
      ${o.notes?`<p><strong>Obs.:</strong> ${esc(o.notes)}</p>`:''}
      <span class="agenda-sos-status">${esc(statusLabel(o.status))}</span>
      <button type="button" class="agenda-sos-open-item" data-open-sos="${esc(o.id)}">Ver na Agenda S.O.S.</button>
    </article>`).join('');
