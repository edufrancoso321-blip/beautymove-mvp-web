...
(function(){
  ...
  function ensureReservation(detail){
    ...
    /* Se a correção de duplicidade marcou a reserva original como cancelada,
       ela deve ser reativada quando a oportunidade S.O.S. continua válida. */
    ...
    if(existing){
      ...
      opportunity.appointmentId=existing.id;
      write(state);return;
    }

    const id=`apt-sos-${Date.now()}-${Math.random().toString(36).slice(2,7)}`;
    if(appointmentConflict(state,date,professional,timeStart,duration,id)){opportunity.status='searching';opportunity.acceptedBy='';opportunity.acceptedAt=null;opportunity.appointmentId='';write(state);const n=document.getElementById('agendaNotice');if(n){n.textContent=`${professional} não está disponível das ${timeStart} às ${opportunity.endTime}.`;n.hidden=false;setTimeout(()=>n.hidden=true,4500);}return;}
