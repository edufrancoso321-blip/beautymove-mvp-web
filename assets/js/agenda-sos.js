...
(function(){
  ...
  function render(){
    ...
    grid.querySelectorAll('.sos-event').forEach(btn=>btn.addEventListener('click',()=>window.dispatchEvent(new CustomEvent('beautymove:sos-selected',{detail:{id:btn.dataset.id}}))));
  }
  function boot(){
    const style=document.createElement('style');style.textContent=`#sosAgendaGrid{overflow:auto;background:#fff;border:1px solid #ece9f0;border-radius:12px}.sos-grid{display:grid;grid-template-columns:88px minmax(520px,1fr);min-width:608px}.sos-times{background:#fff;border-right:1px solid #ece9f0}.sos-time{height:60px;box-sizing:border-box;border-bottom:1px solid #ece9f0;padding:10px 8px;text-align:center;color:#2c2733;font-size:12px}.sos-lane{position:relative;background:repeating-linear-gradient(to bottom,#ece9f0 0,#ece9f0 1px,transparent 1px,transparent 60px)}.sos-event{position:absolute;left:8px;right:8px;box-sizing:border-box;border:0;border-left:4px solid #7438ff;border-radius:8px;background:#f2eaff;padding:8px 10px;text-align:left;cursor:pointer;color:#211b29}.sos-event strong,.sos-event span,.sos-event small{display:block}.sos-event strong{font-size:14px}.sos-event span{font-size:12px;margin-top:3px}.sos-event small{font-size:10px;margin-top:3px;color:#5d5269}.sos-empty{position:absolute;top:20px;left:20px;color:#7b7382;font-size:13px}@media(max-width:780px){#sosAgendaGrid{max-height:calc(100vh - 220px)}}`;document.head.appendChild(style);
