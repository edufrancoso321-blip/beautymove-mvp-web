 ...
 * Este módulo é somente visual/orquestrador da Central.
 * Regra estrutural: ele NÃO corrige nem reescreve appointment.professional.
 * A confirmação da profissional pertence exclusivamente ao fluxo S.O.S. de reserva.
 */
(function(){
  'use strict';
  ...
  function loadVisual(){if(document.getElementById('sosPanelVisualV2'))return;const link=document.createElement('link');link.id='sosPanelVisualV2';link.rel='stylesheet';link.href='assets/css/sos-opportunity-panel-v2.css?v=20260818-1';document.head.appendChild(link);}
  ...
  function affected(){
    ...
    return appointments.filter(a=>a&&a.date===date&&a.status!=='cancelado'&&!a.sosAcceptedBy).map(a=>{
      const r=statuses[date+'::'+a.professional];if(!r||!['absent','late'].includes(r.status))return null;
      const start=minutes(a.time),end=start+Math.max(30,Number(a.duration)||60);
      const ok=r.status==='late'?start<minutes(r.lateStart||'00:00'):r.absenceType==='during_day'?end>minutes(r.absenceStart||'23:59'):true;
      if(!ok)return null;
      return {id:'affected-'+a.id,appointmentId:a.id,date:a.date,time:a.time,client:a.client||'Cliente',service:a.service||'Atendimento',specialty:specialty[a.professional]||'Cabelos',professional:a.professional||'',kind:'urgente',status:'searching',reason:r.status==='late'?'Atraso':'Ausência'};
    }).filter(Boolean);
  }

  function manual(){
    const date=today(),state=read(STATE_KEY,{opportunities:[]}),items=Array.isArray(state.opportunities)?state.opportunities:[];
    return items.filter(o=>o&&o.date===date&&o.source==='sos'&&o.status!=='cancelado'&&o.status!=='resolved').map(o=>({...o,kind:o.kind||'manual',status:o.status||'searching'}));
  }
  ...
  function tracking(){
    ...
    const tracked=items.filter(o=>o&&o.date===date&&o.source==='sos'&&o.status==='resolved'&&o.acceptedBy).map(o=>{
      const appointment=o.appointmentId?appointments.find(a=>a&&a.id===o.appointmentId):null;
      return {...o,kind:'tracking',status:'tracking',professional:o.acceptedBy,appointmentStatus:appointment?.status||'confirmado'};
    });
  ...
  function opportunities(){
    const map=new Map();
    [...affected(),...manual()].forEach(item=>{const key=item.appointmentId||item.id;if(!map.has(key)||item.kind==='manual')map.set(key,item);});
    return [...map.values()].sort((a,b)=>minutes(a.time)-minutes(b.time));
  }

  function candidateList(){
    return candidates.map(c=>`<div class="sos-op-candidate"><div class="sos-op-avatar-placeholder">${esc(c[0].charAt(0))}</div><div class="sos-op-candidate-main"><div class="sos-op-candidate-name">${esc(c[0])} <span>★ ${c[1]}</span></div><div class="sos-op-candidate-data">◷ ${c[2]} · ⌖ ${c[3]}</div><div class="sos-op-available">● Disponível</div></div><button type="button" class="sos-op-select" data-professional="${esc(c[0])}">Selecionar</button></div>`).join('');
  }
  ...
  function selectCandidate(professional){
    ...
    const items=opportunities(),active=items[0];if(!active)return;
    const now=new Date().toISOString();

    if(active.appointmentId){
      const appointment=state.appointments.find(a=>a&&a.id===active.appointmentId);
      if(appointment){
        /* A profissional confirmada é a profissional efetiva do atendimento. */
        appointment.professional=professional;
        appointment.sosAcceptedBy=professional;
        appointment.sosAcceptedAt=now;
      }
      state.opportunities.forEach(o=>{
        if(o&&o.source==='sos'&&o.date===active.date&&o.time===active.time&&o.client===active.client){
          o.status='resolved';o.acceptedBy=professional;o.acceptedAt=now;o.appointmentId=o.appointmentId||active.appointmentId;
        }
      });
  ...
  function render(){
    ...
    const items=opportunities(),tracked=tracking();
    if(!items.length&&!tracked.length){
      panel.innerHTML=`<div class="sos-op-shell"><header class="sos-op-header"><div class="sos-op-kicker"><span class="sos-op-bolt">⚡</span><span>S.O.S.</span></div><div class="sos-op-subtitle">Central de Oportunidades</div><div class="sos-op-state"><span></span>Pronto para agir</div></header><div class="sos-op-body"><div class="sos-op-empty"><div class="sos-op-empty-icon">✓</div><strong>Tudo sob controle</strong><span>Nenhuma oportunidade ativa.</span></div></div></div>`;
      return;
    }
    if(!items.length&&tracked.length){
      const item=tracked[0];
      panel.innerHTML=`<div class="sos-op-shell tracking"><header class="sos-op-header active"><div class="sos-op-kicker"><span class="sos-op-bolt">⚡</span><span>S.O.S.</span></div><div class="sos-op-subtitle">Central de Oportunidades</div><div class="sos-op-state active"><span></span>Acompanhando atendimento</div></header><div class="sos-op-body"><section class="sos-op-alert"><div class="sos-op-alert-label">PROFISSIONAL CONFIRMADA</div><strong>${esc(item.client||'Cliente')}</strong><div>${esc(item.service||'Atendimento')} · ${esc(item.time||'A definir')}</div><small>${esc(item.acceptedBy||item.professional||'A definir')}</small></section><div class="sos-op-search-state"><span class="sos-op-search-dot"></span><strong>Atendimento em acompanhamento</strong><small>${esc(item.specialty||'Especialidade')}</small></div><div class="sos-op-candidate sos-op-selected-candidate"><div class="sos-op-avatar-placeholder">${esc((item.acceptedBy||'P').charAt(0))}</div><div class="sos-op-candidate-main"><div class="sos-op-candidate-name">${esc(item.acceptedBy||'Profissional')}</div><div class="sos-op-candidate-data">Profissional confirmada para o atendimento</div><div class="sos-op-available">● Em acompanhamento</div></div></div><div class="sos-op-followup">A oportunidade S.O.S. foi encerrada. O atendimento continua visível na Agenda e sendo acompanhado nesta Central.</div></div><footer class="sos-op-footer">✓ Acompanhamento ativo</footer></div>`;
      return;
    }
  ...
  function boot(){
    loadVisual();ensureWorkspace();render();
    let signature='';
    const refresh=()=>{const s=JSON.stringify([today(),localStorage.getItem(STATUS_KEY),localStorage.getItem(STATE_KEY)]);if(s!==signature){signature=s;render();}};
    window.addEventListener('storage',event=>{if(event.storageArea===localStorage&&(event.key===STATE_KEY||event.key===STATUS_KEY))refresh();});
    window.addEventListener('beautymove:sos-runtime-refresh',refresh);
    window.addEventListener('beautymove:agenda-hydrated',refresh);
    document.getElementById('agendaDatePicker')?.addEventListener('change',()=>{signature='';refresh();});
    document.getElementById('prevDay')?.addEventListener('click',()=>setTimeout(()=>{signature='';refresh();},150));
    document.getElementById('nextDay')?.addEventListener('click',()=>setTimeout(()=>{signature='';refresh();},150));
    document.getElementById('todayBtn')?.addEventListener('click',()=>setTimeout(()=>{signature='';refresh();},150));
  }
