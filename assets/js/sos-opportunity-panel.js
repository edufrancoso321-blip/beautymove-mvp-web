...
(function(){
  ...
  function manual(){const date=today(),state=read(STATE_KEY,{opportunities:[]}),items=Array.isArray(state.opportunities)?state.opportunities:[];return items.filter(o=>o&&o.date===date&&o.source==='sos'&&!['resolved','cancelado'].includes(o.status)).map(o=>({...o,kind:o.kind||'manual',status:o.status||'searching'}));}
